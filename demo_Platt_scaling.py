...
def demo(data, save, depth=40, growth_rate=12, batch_size=256):
    ...
    # Set model to state_dict

    # Now we're going to wrap the model with a decorator that adds Platt scaling
    model = ModelWithPlatt(orig_model)

    # Tune the model temperature, and save the results
    model.set_a_b(valid_loader)
    model_filename = os.path.join(save, 'model_with_Platt.pth')
    torch.save(model.state_dict(), model_filename)
    print('Platt scaled model sved to %s' % model_filename)
    print('Done!')
