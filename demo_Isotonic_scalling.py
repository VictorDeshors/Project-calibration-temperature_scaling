...
def demo(data, save, depth=40, growth_rate=12, batch_size=256):
    ...
    Applies Isotonic scaling, and saves a Isotonic scaled version.
    ...
    # Now we're going to wrap the model with a decorator that adds Isotonic scaling
    model = ModelWithIsotonic(orig_model)

    # Tune the model temperature, and save the results
    model.set_f(valid_loader)
    model_filename = os.path.join(save, 'model_with_Isotonic.pth')
    torch.save(model.state_dict(), model_filename)
    print('Isotonic scaled model sved to %s' % model_filename)
    print('Done!')
